...
def generar_rango_60a140(): 
    num1 = random.randint(60, 120)  #### Asegura un rango válido PARA LA TENDECIA DE LA LINEA DERECHA ####
    num2 = num1 + 10
    if num2 > 140:
        num2 = 120
    return (num1, num2)
...
folder_path = 'C:/Users/USER/Desktop/Aplicacion/GR_entrenamiento_rand/'
file_prefix = 'GR_entrenamiento_rand'
...
for i in range(10):
    ...
    #### Conjuntos menores a 60 #### #### ES POSIBLE CAMBIAR VALORES DE RANGO PARA CAMBIAR LA TENDENCIA HACIA LA IZQUIERDA
    set1 = np.sort(np.random.uniform(rango_1, rango_4 + 1, set_size))
    set2 = np.sort(np.random.uniform(rango_3, rango_2 + 1, set_size))[::-1]
    set3 = np.sort(np.random.uniform(rango_1, rango_4 + 1, set_size))
    set4 = np.sort(np.random.uniform(rango_3, rango_2 + 1, set_size))[::-1]
    ...
    #### Concatenar todos los conjuntos de datos #### ES POSIBLE ORDENAR para cambiar las tendecias a la izquierda.
